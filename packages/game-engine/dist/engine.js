...
exports.applyAction = exports.getLegalMoves = exports.getView = exports.createGame = exports.makeDeck = void 0;
// -------------------------------------------------------
// Utils
// -------------------------------------------------------
const suits = ["♠", "♥", "♦", "♣"];
...
const makeDeck = () => {
    ...
    for (const s of suits) {
        for (let r = 2; r <= 14; r++) {
            deck.push({ id: `${r}${s}-${uid()}`, rank: r, suit: s });
        }
    }
    return deck;
};
...
const createGame = (opts) => {
    ...
    const deck = shuffle((0, exports.makeDeck)(), opts.seed);
...
// Légalité d’un set
// -------------------------------------------------------
const canPlaySet = (S, P, cards) => {
    if (cards.length === 0)
        return "Aucune carte sélectionnée.";
    if (!allSameRank(cards))
        return "Toutes les cartes jouées doivent être du même rang.";
    // cartes doivent appartenir au joueur
    const own = new Set(P.hand.map((c) => c.id));
    for (const c of cards)
        if (!own.has(c.id))
            return "Carte non possédée.";
    const r = cards[0].rank;
    if (S.le7Active) {
        // Sous contrainte ≤7 : seules cartes de rang ≤7 autorisées, effets spéciaux annulés
        if (r > 7)
            return "Sous contrainte ≤7 : vous devez jouer une carte de rang ≤ 7.";
        return null;
    }
    // Mode normal (pas de ≤7)
    const topCard = top(S.tablePile);
    const required = Math.max(S.threshold, topCard ? topCard.rank : S.threshold);
    if (r < required) {
        return `Vous devez jouer ≥ ${rankName(required)} (top ${topCard ? rankName(topCard.rank) : rankName(S.threshold)}).`;
    }
    return null;
};
// -------------------------------------------------------
...
const getLegalMoves = (S, playerId) => {
...
// -------------------------------------------------------
const applyAction = (S, playerId, action) => {
    if (S.status !== "playing")
        return S;
    const idx = S.players.findIndex((p) => p.id === playerId);
    if (idx === -1 || idx !== S.current)
        return S;
    ...
    if (action.kind === "pickup") {
        if (S.tablePile.length === 0)
            return S; // rien à ramasser
    ...
    if (!burned) {
        ...
        if (topCard) {
            ...
            if (specialsSuppressed || (rank !== 2 && rank !== 11)) {
                S.threshold = Math.max(S.threshold, topCard.rank);
            }
        }
    }
